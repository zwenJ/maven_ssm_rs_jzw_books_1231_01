...
<div class="w960">
    ...
    <div class="w740 right">
        ...
        <jsp:include page="../../common/select_order.jsp" flush="true">
            <jsp:param name="SelectOrder" value="order/showorder/user/orders/order1.controller?status=2&bookType=${BooksZxPtUser}"/>
        </jsp:include>
        ...
        <div class="order02">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <c:if test="${not empty MyOrderList}">
                    ...
                    <c:forEach items="${MyOrderList}" var="order">
                        <tr style="font-size: 12px">
                            ...
                            <td width="10%" class="text">
                                <a href="javascript:fun_update_order('${order.orderId}',7)">撤单</a>
                            </td>
                        </tr>
                    </c:forEach>
                ...
                <script>
                    function fun_update_order(orderid, value){
                        $.post("/books/order/updateorder.controller",{
                            ...
                            if ("success" == _result) {
                                alert("操作成功！");
                                location.replace("/books/order/showorder/user/orders/order1.controller?status=2&bookType=${BooksZxPtUser}");
                            }
                            else {
                                alert("操作失败！");
                            }
                ...
                <c:if test="${empty MyOrderList}">
                    <tr style="font-size: 12px">
                        <td colspan="5" style="text-align: center">暂无未发货订单</td>
                    </tr>
                </c:if>
        ...
        <c:if test="${not empty MyOrderList}">
            <jsp:include page="../../common/page.jsp" flush="true">
                <jsp:param name="pathPage" value="order/showorder/user/orders/order1.controller?status=2&bookType=${BooksZxPtUser}"/>
            </jsp:include>
        </c:if>
